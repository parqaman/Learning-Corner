...
import { DI } from '..';
import { CreateGroupDTO, CreateGroupSchema, CreateSectionDTO, CreateSectionSchema, Group, Section } from '../entities';
...
router.post('/', async (req, res) => {
  const validatedData = await CreateGroupSchema.validate(req.body).catch((e) => {
...
router.get('/:groupId/message', async (req, res) => {
  ...
  if (!learnerInGroup) {
    return res.status(401).send({ message: 'You are not the member of this group' });
  }
  const messages = await DI.messageRepository.find({ roomId: group.id }, { populate: ['sender'] });
