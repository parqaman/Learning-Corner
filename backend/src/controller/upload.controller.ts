...
function handleNewFile(section: Section, tmpName: string, fileName: string): string {
    ...
    function getUnusedFileName(x: number, fileName: string): string {
        const filesWithSameFileName = section.files.getItems().filter(f => f.name === fileName);
        if(filesWithSameFileName.length > 0){
            return getUnusedFileName(x++, x + '_' + fileName)
        }
        return fileName;
    }
