  ...
  return (
    <Flex m={'0.75rem auto'} justifyContent={'flex-start'}>
      <Flex flexDir="column" display={'inline-flex'} alignItems="flex-start">
        <Text bg="#E2E8F0" p="0.25rem 0.75rem" borderRadius={'0 1.2rem 1.2rem 1.2rem'}>
          {message.message}
        </Text>
  ...
  return (
    <Flex m={'0.75rem auto'} justifyContent={'flex-end'}>
      <Flex flexDir="column" display={'inline-flex'} alignItems="flex-end">
        <Text bg="#D5D8DC" p="0.25rem 0.75rem" borderRadius={'1.2rem 1.2rem 0 1.2rem'} flex={1}>
          {message.message}
        </Text>
        <Text as={'span'} alignSelf={'flex-end'} p="0rem 0.5rem" fontSize={'small'} color={'rgba(0, 0, 0, 0.85)'}>
          You - {message.time && new Date(parseInt(message.time)).toLocaleTimeString()}
        </Text>
      </Flex>
    </Flex>
  );
...
export const ChatWindow = ({ cardID, roomID }: { cardID: string; roomID: string }) => {
  ...
  const getChatHistory = async () => {
    if (cardID === 'course-card') {
      await apiClient.getCourseMessage(roomID).then((res) => {
        setMessages(res.data);
      });
    } else {
      await apiClient.getGroupMessage(roomID).then((res) => {
        setMessages(res.data);
      });
    }
  };
  ...
  useEffect(() => {
    if (!socket) {
      ...
      socket.on('connect', () => {
        socket.emit('helloRoom', { user, room: roomID });
      });
  ...
  return (
    <Flex
      ...
      flexDir="column"
    >
      <Flex alignItems={'center'} justifyContent="space-between" borderBottom={'0.08rem solid #0194F3'}>
        ...
        <Text
          ...
          alignItems="center"
          cursor={'pointer'}
          onClick={() => ActionClose(cardID)}
        >
          <AiOutlineMinus />
        </Text>
      </Flex>
      <Box id="chat-contents" overflowY="scroll" m="0.5rem auto" flex={1} width={'100%'}>
        {messages.map((message) => (
          <Box key={message.time} id={message.time}>
            {message.sender && message.sender.id === user.id ? (
              <OutcomingSingleChatTile message={message} />
            ) : (
              <IncomingSingleChatTile message={message} />
            )}
          </Box>
        ))}
      </Box>
      <Flex columnGap={'0.5rem'} borderRadius="2rem" p={'0.5rem'} alignItems={'center'} bg="#D5D8DC" bottom={'0'}>
        <Input
          alignItems={'center'}
          pl="0.5rem"
          variant="unstyled"
