...
export const FavoriteCourseList = ({courses}: {courses: Course[]}) => {
    ...
    const handleNextPage = () => {
        if(courses && courses.length > 0) {
            if(offset+MAX_NUM_OF_SHOWN_COURSES < courses.length) { //already at the bottom of the list
    ...
    const handlePreviousPage = () => {
        if(courses && courses.length > 0) {
            if(offset !== 0) { //already on the top of the list
                const localOffset = offset - 3
                setOffset((oldVal) => oldVal - 3)
                const slicedArray = courses.slice(localOffset, MAX_NUM_OF_SHOWN_COURSES-localOffset)
                setShownCourses(slicedArray)
    ...
    return (
        <Box w={'47rem'} flexDir={'column'} alignItems={'center'} mt='0.25rem' mb={'1.5rem'}>
            ...
            <Flex gap='0.5rem' justifyContent='flex-end' pt={'0.5rem'} >
                <Text onClick={handlePreviousPage} cursor={'pointer'} bg={'rgba(0, 0, 0, 0.75)'} borderRadius='full' color='white' p='0.25rem' >
                    <AiOutlineLeft/>
                </Text>
                <Text onClick={handleNextPage} cursor={'pointer'} bg={'rgba(0, 0, 0, 0.75)'} borderRadius='full' color='white' p='0.25rem' >
                    <AiOutlineRight/>
                </Text>
            </Flex>
        </Box>
