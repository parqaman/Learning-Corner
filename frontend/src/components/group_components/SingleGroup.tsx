...
export const SingleGroup = ({ course, group, myGroupPage }: { course: Course; group: Group; myGroupPage: boolean }) => {
  ...
  const fetchData = async () => {
    ...
    if (currentUser) {
      await apiClient.getUsersUseridGroups(currentUser.id).then((res) => {
        ...
        if (group.id) {
          groupID.includes(group.id) ? setJoined(true) : setJoined(false);
        }
      });
    }
  };
  ...
  const joinGroup = async () => {
    if (currentUser && course && course.id && group && group.id) {
      await apiClient.putUsersUseridCourseCourseidGroupGroupid(currentUser.id, course.id, group.id).then((res) => {
        toast({
          title: 'Joined',
  ...
  const leaveGroup = async () => {
    if (currentUser && course && course.id && group && group.id) {
      await apiClient.deleteUsersUseridCourseCourseidGroupGroupid(currentUser.id, course.id, group.id).then((res) => {
        toast({
          title: 'Left',
