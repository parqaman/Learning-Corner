...
export const AppLayout = (props: AppLayoutProps) => {
  ...
  const HamburgerMenuItems = () => {
    return (
      ...
      >
        ...
        <Link to={'/home'}>
          <NavItem _hover={{ background: 'rgba(0, 0, 0, 0.1)', borderRadius: '0.2rem' }}>Home</NavItem>
        </Link>
        <Link to={'/mycourses'}>
          <NavItem _hover={{ background: 'rgba(0, 0, 0, 0.1)', borderRadius: '0.2rem' }}>My Courses</NavItem>
        </Link>
        <Link to={'/mygroups'}>
          <NavItem _hover={{ background: 'rgba(0, 0, 0, 0.1)', borderRadius: '0.2rem' }}>My Groups</NavItem>
        </Link>
        <Link to={'/profile'}>
          <NavItem _hover={{ background: 'rgba(0, 0, 0, 0.1)', borderRadius: '0.2rem' }}>My Profile</NavItem>
        </Link>
      </Flex>
    );
  };
  ...
  const navigationItems = (
    <>
      <Flex display={{ base: 'none', sm: 'flex' }} justifyContent={'space-between'} width={'100%'}>
        ...
        <Menu isOpen={profileDisclosure.isOpen}>
          <MenuButton
            ...
            onMouseEnter={profileDisclosure.onOpen}
            onMouseLeave={profileDisclosure.onClose}
            p={'0.25em'}
            _hover={{ background: 'rgba(0, 0, 0, 0.1)', borderRadius: '0.2rem' }}
          >
            <Link to={'/profile'}>Profile</Link>
          </MenuButton>
          <MenuList
            style={{ background: 'white', marginTop: '-0.5rem', boxShadow: '0 0 1rem 0.1rem rgba(0, 0, 0, 0.1)' }}
            onMouseEnter={profileDisclosure.onOpen}
            onMouseLeave={profileDisclosure.onClose}
          >
            <Link to={'/profile'}>
            ...
            <Link to={'/'}>
              <NavItem _hover={{ background: 'rgba(0, 0, 0, 0.1)' }} fontSize={'sm'} onClick={useLogout}>
                Logout
              </NavItem>
            </Link>
          </MenuList>
      ...
      <Flex display={{ base: 'flex', sm: 'none' }} onClick={hamburgerDisclosure.onOpen}>
