...
export const CourseDetailPage = () => {
  ...
  const fetchData = async () => {
    await apiClient
      ...
      .then(async (res) => {
        ...
        if (currentUser) {
          await apiClient.getUserFavoriteCourses(currentUser.id).then((res) => {
  ...
  const handleDeleteCourse = async () => {
    if (course && currentUser) {
      const res = await apiClient
        .deleteCoursesCourseIDUsersUserID(course.id!)
        .then(() => {
          toast({
            title: 'Deleted',
  ...
  const handleNewSection = async () => {
    if (course) {
      if (course && course.id) {
        await apiClient
          .postSectionCourse(course.id, newSection)
          .then((response) => {
  ...
  const handleNewGroup = async () => {
    if (newGroup) {
      await apiClient.postGroups(newGroup).then((res) => {
  ...
  const joinCourse = async () => {
    if (course && currentUser) {
      const res = await apiClient
        .putUsersUserIDCourseCourseID(currentUser.id, course.id!)
        .then(() => {
          toast({
            title: 'Joined',
  ...
  const leaveCourse = async () => {
    if (course && currentUser) {
      const res = await apiClient
        .deleteUsersUserIDCourseCourseID(currentUser.id, course.id!)
        .then(() => {
        ...
        </Flex>
        {
          ...
          course ? (
            <CourseDescriptionSection
              ...
              updateHandler={handleEditCourseInfo}
              isOwner={isOwner}
            />
          ) : (
        ...
        {(joined || isOwner) && (
          <Box mt={'2rem'}>
            ...
            <Box mb={'1rem'}>
              ...
              {/** New group modal */}
              <Button
                onClick={newGroupDisclosure.onOpen}
                variant={'solid'}
                _hover={{}}
                _active={{}}
                size="xs"
            ...
            {course && <GroupList course={course} groups={groups} />}
          </Box>
        )}
