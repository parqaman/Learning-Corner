...
const FilterButton = ({courses, setCourses, user}: {courses: Course[] | undefined; setCourses: React.Dispatch<React.SetStateAction<Course[] | undefined>>; user: User | null}) => {
  ...
  return(
    <Flex alignItems={'center'} gap='0.5rem' mb={'0.5rem'}>
      ...
      <Switch id='switch' onChange={(e)=>{handleFilterCourse(e)}} />
    </Flex>
  )
}
    ...
    <AppLayout display={'flex'} flexDir='column' alignItems='center' mt={'3rem'}>
      <SlideFade delay={0.2} in={true}>
        <Box display='flex' flexDirection={'column'}>
            ...
            <Box><FilterButton courses={courses} setCourses={setCourses} user={user}/></Box>
