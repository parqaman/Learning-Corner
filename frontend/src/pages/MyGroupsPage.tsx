  ...
  return (
    <Box display="flex" flexDir={'column'} alignItems="center" justifyContent={'flex-end'}>
      <Menu closeOnSelect={true}>
        <MenuButton _hover={{ textDecoration: 'underline' }}>Filter by course</MenuButton>
        <MenuList>
          <MenuOptionGroup defaultValue="no-filter" type="radio">
            <MenuItemOption value="no-filter" onClick={(e) => handleGroupFilter(e)}>
              No filter
            </MenuItemOption>
            {courses &&
              courses.map((course) => (
                <MenuItemOption key={course.id} value={course.id} onClick={(e) => handleGroupFilter(e)}>
                  {course.name}
                </MenuItemOption>
              ))}
          </MenuOptionGroup>
            ...
            {groups && groups.length > 0 ? (
              groups
                .sort((a, b) =>
                  ...
                  a.group.course &&
                  a.group.course.name &&
                  a.group.course.name.toLocaleLowerCase() > b &&
                  b.group &&
                  b.group.course &&
                  b.group.course.name &&
                  b.group.course.name.toLocaleLowerCase()
                    ? 1
                    : -1,
                )
                .map(
                  (val) =>
                    val.group &&
                    val.group.course && (
                      <SingleGroup key={val.group.id} course={val.group.course} group={val.group} myGroupPage={true} />
                    ),
                )
            ) : (
