      ...
      await apiClient
        ...
        .then(async (res) => {
          if (user) {
            ...
            if (theCourse.id) {
              await apiClient
                .putUsersUserIDCourseCourseID(user.id, theCourse.id)
                .then((e) => {
                  console.log(e);
                  toast({
                    title: 'Sucessful',
      ...
      <NewCourseCard>
        ...
        <form onSubmit={(e) => handleNewCourse(e)} data-testid="form">
          <Box width={'100%'} gap={'1em'} display="flex" flexDirection={'column'}>
            <Input
              value={course?.name}
              onChange={(e) => setCourse({ name: e.target.value, lecturer: user!, description: course?.description })}
              as={Input}
              isRequired
              padding={'0.75em'}
              variant={'unstyled'}
            ...
            <Textarea
              ...
              onChange={(e) => setCourse({ name: course?.name, lecturer: user!, description: e.target.value })}
              isRequired
              padding={'0.75em'}
