...
const ProfileDetailTile = ({ description, data, edit, editDetail, setEditDetail, editHandler }: ProfileProps) => {
  return (
    <Box
      p="0.25rem 0.5rem 0.5rem 0.5rem"
...
export const ProfilePage = () => {
  ...
  const ProfilePhoto = () => {
    if (photo) {
      return <Image src={photo} h={'7.5rem'} w={'7.5rem'} objectFit={'cover'} />;
    }
    if (user?.photo) {
      return (
        <Image
          src={'http://localhost:4000/upload/profile/' + user?.photo}
          h={'7.5rem'}
          w={'7.5rem'}
          objectFit={'cover'}
        />
